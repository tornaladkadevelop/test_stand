...
def st_test_tzp():
    ...
    fault.debug_msg("напряжение \t" + str(meas_volt), 3)
    ...
    k = 0
    for i in list_ust:
        mysql_conn.mysql_ins_result('проверка уставки ' + str(list_ust_num[k]), '3')
        msg_5 = ("Установите регулятор уставок на блоке в положение \t" + str(list_ust_num[k]))
        ...
        if 0.9 * i / coef_volt <= meas_volt <= 1.1 * i / coef_volt:
            ...
            fault.debug_msg("напряжение соответствует " + str(meas_volt), 3)
            ctrl_kl.ctrl_relay('KL63', True)
            ...
            fault.debug_msg("тест 3 delta t " + str(calc_delta_t), 4)
