...
def st_test_bkz_zmk():
    ...
    fault.debug_msg("коэффициент сети \t" + str(coef_volt), 3)
    ...
    reset.sbros_zashit_kl30_1s5()
    in_a5, in_a6 = __inputs_a()
    fault.debug_msg("положение входов \t" + str(in_a5) + str(in_a6), 5)
    if in_a5 == True and in_a6 == True:
        pass
    else:
        return False
    ...
    for i in list_ust_mtz:
        # k += 1
        msg_result_mtz = my_msg_2(msg_3 + str(list_ust_mtz_num[k]))
        if msg_result_mtz == 0:
            pass
        elif msg_result_mtz == 1:
            return False
        elif msg_result_mtz == 2:
            mysql_conn.mysql_add_message('уставка МТЗ ' + str(list_ust_mtz_num[k]) + ' пропущена')
        ...
        fault.debug_msg("напряжение \t" + str(meas_volt_test4), 2)
        ...
        if calc_delta_t_mtz != 9999:
            ...
            mysql_conn.mysql_ins_result('неисправен', '4')
        fault.debug_msg("дельта t \t" + str(calc_delta_t_mtz), 2)
        list_delta_t_mtz.append(round(calc_delta_t_mtz, 0))
        mysql_conn.mysql_add_message('уставка МТЗ ' + str(list_ust_mtz_num[k]) +
                                     ' дельта t: ' + str(calc_delta_t_mtz))
        in_a5, in_a6 = __inputs_a()
        fault.debug_msg("положение входов \t" + str(in_a5) + str(in_a6), 5)
    ...
    for n in list_ust_tzp:
        ...
        msg_result_tzp = my_msg(msg_5 + str(list_ust_tzp_num[m]))
        if msg_result_tzp == 0:
            pass
        ...
        fault.debug_msg("напряжение \t" + str(meas_volt_test5), 2)
        ...
        calc_delta_t_tzp = __delta_t_tzp()
        fault.debug_msg("дельта t \t" + str(calc_delta_t_tzp), 2)
        list_delta_t_tzp.append(round(calc_delta_t_tzp, 0))
        mysql_conn.mysql_add_message('уставка ТЗП ' + str(list_ust_tzp_num[m]) +
                                     ' дельта t: ' + str(calc_delta_t_tzp))
