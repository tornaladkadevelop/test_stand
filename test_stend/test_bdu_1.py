#!/usr/bin/env python3
# -*- coding: utf-8 -*-

"""
Алгоритм проверки
Тип блока Производитель
БДУ-1	Без Производителя
БДУ-1	Углеприбор

"""

from sys import exit
from time import sleep
from gen_func_utils import *
from my_msgbox import *
from gen_mb_client import *
from gen_mysql_connect import *

__all__ = ["st_test_bdu_1"]

reset = ResetRelay()
resist = Resistor()
result = Result()
ctrl_kl = CtrlKL()
read_mb = ReadMB()
mysql_conn = MySQLConnect()
fault = Bug(True)


def st_test_bdu_1():
    # reset.reset_all()
    in_a1 = __inputs_a()
    if in_a1 == False:
        mysql_conn.mysql_ins_result('исправен', '1')
        fault.debug_msg('in_01' + str(in_a1) + 'тест 1 пройден', 3)
    else:
        fault.debug_msg('in_01 ' + str(in_a1) + 'блок неисправен', 1)
        mysql_conn.mysql_ins_result('неисправен', '1')
        return False
    # Тест-2 Проверка включения/отключения блока от кнопки пуск
    # Включение KL2
    ctrl_kl.ctrl_relay('KL2', True)
    sleep(3)
    # Тест-2.1 Проверка исходного состояния
    in_a1 = __inputs_a()
    if in_a1 == False:
        fault.debug_msg('in_01' + str(in_a1) + 'тест 2.1 пройден', 3)
    else:
        fault.debug_msg('in_01 ' + str(in_a1) + 'блок неисправен', 1)
        mysql_conn.mysql_ins_result('неисправен', '2')
        return False
    # Тест-2.2 Проверка канала блока от кнопки "Пуск"
    #  формируем 10 Ом
    resist.resist_ohm(10)
    sleep(3)
    ctrl_kl.ctrl_relay('KL12', True)
    sleep(3)
    in_a1 = __inputs_a()
    if in_a1 == True:
        fault.debug_msg('in_01' + str(in_a1) + 'тест 2.2 пройден', 3)
    else:
        fault.debug_msg('in_01 ' + str(in_a1) + 'Тест 2.2 блок неисправен', 1)
        mysql_conn.mysql_ins_result('неисправен', '2')
        return False
    # Тест 2.3 Выключение канала блока от кнопки «Пуск» при сопротивлении 10 Ом
    sleep(3)
    ctrl_kl.ctrl_relay('KL12', False)
    sleep(3)
    in_a1 = __inputs_a()
    if in_a1 == False:
        fault.debug_msg('in_01' + str(in_a1) + 'тест 2.3 пройден', 3)
        mysql_conn.mysql_ins_result('исправен', '2')
    else:
        fault.debug_msg('in_01 ' + str(in_a1) + 'Тест 2.3 блок неисправен', 1)
        mysql_conn.mysql_ins_result('неисправен', '2')
        return False
    # Тест-3. Удержание исполнительного элемента при сопротивлении цепи заземления до 35 Ом
    sleep(3)
    ctrl_kl.ctrl_relay('KL12', True)
    sleep(0.5)
    # Отключаем KL5, KL8 для формирования 35 Ом
    ctrl_kl.ctrl_relay('KL5', False)
    ctrl_kl.ctrl_relay('KL8', False)
    sleep(1)
    in_a1 = __inputs_a()
    if in_a1 == False:
        fault.debug_msg('in_01' + str(in_a1) + 'тест 3 пройден', 3)
        mysql_conn.mysql_ins_result('исправен', '3')
    else:
        fault.debug_msg('in_01 ' + str(in_a1) + 'Тест 3 блок неисправен', 1)
        mysql_conn.mysql_ins_result('неисправен', '3')
        return False
    # Тест 4. Отключение исполнительного элемента при сопротивлении цепи заземления свыше 50 Ом
    ctrl_kl.ctrl_relay('KL7', False)
    ctrl_kl.ctrl_relay('KL9', False)
    ctrl_kl.ctrl_relay('KL4', True)
    ctrl_kl.ctrl_relay('KL6', True)
    ctrl_kl.ctrl_relay('KL10', True)
    sleep(2)
    in_a1 = __inputs_a()
    if in_a1 == False:
        fault.debug_msg('in_01' + str(in_a1) + 'тест 4 пройден', 3)
        mysql_conn.mysql_ins_result('исправен', '4')
    else:
        fault.debug_msg('in_01 ' + str(in_a1) + 'Тест 4 блок неисправен', 1)
        mysql_conn.mysql_ins_result('неисправен', '4')
        return False
    ctrl_kl.ctrl_relay('KL12', True)
    sleep(0.5)
    # Тест 5. Защита от потери управляемости при замыкании проводов ДУ
    #  формируем 10 Ом
    resist.resist_ohm(10)
    sleep(1)
    ctrl_kl.ctrl_relay('KL12', True)
    sleep(0.5)
    ctrl_kl.ctrl_relay('KL11', True)
    sleep(1)
    in_a1 = __inputs_a()
    if in_a1 == False:
        fault.debug_msg('in_01' + str(in_a1) + 'тест 5 пройден', 3)
        mysql_conn.mysql_ins_result('исправен', '5')
    else:
        fault.debug_msg('in_01 ' + str(in_a1) + 'Тест 5 блок неисправен', 1)
        mysql_conn.mysql_ins_result('неисправен', '5')
        return False
    ctrl_kl.ctrl_relay('KL12', False)
    ctrl_kl.ctrl_relay('KL11', False)
    ctrl_kl.ctrl_relay('KL1', False)
    # Тест 6. Защита от потери управляемости при обрыве проводов ДУ
    #  формируем 10 Ом
    resist.resist_ohm(10)
    sleep(1)
    ctrl_kl.ctrl_relay('KL12', True)
    # opc['Устройство.tg.in_pusk_KL12'] = True
    sleep(1)
    ctrl_kl.ctrl_relay('KL12', False)
    sleep(2)
    in_a1 = __inputs_a()
    if in_a1 == False:
        fault.debug_msg('in_01' + str(in_a1) + 'тест 6 пройден', 3)
        mysql_conn.mysql_ins_result('исправен', '6')
        fault.debug_msg('in_01' + str(in_a1) + 'тест завершен', 3)
    else:
        fault.debug_msg('in_01 ' + str(in_a1) + 'Тест 6 блок неисправен', 1)
        mysql_conn.mysql_ins_result('неисправен', '6')
        return False
    return True


def __inputs_a():
    in_a1 = read_mb.read_discrete(1)
    return in_a1


if __name__ == '__main__':
    try:
        if st_test_bdu_1():
            mysql_conn.mysql_block_good()
            my_msg('Блок исправен')
        else:
            mysql_conn.mysql_block_bad()
            my_msg('Блок неисправен')
    except OSError:
        my_msg("ошибка системы")
    except SystemError:
        my_msg("внутренняя ошибка")
    finally:
        reset.reset_all()
        exit()
